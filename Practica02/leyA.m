...
for i = 1 : 401
    ...
    if abs (zNorm(i))< parm
       funComp(i) = (A.*zNorm(i))./(1+log(A)) .*sign(zNorm(i));
        
    else
        funComp(i) = (1 + log(abs(zNorm(i))* A))./(1+log(A)) .*sign(zNorm(i));
    end
...
for i = 1 : 400
    ...
    if abs(funComp(i)) < param2
       funDes(i) = sign(funComp(i)) .* ((abs(funComp(i)) .* (1 + log(A))) ./ A);
        
    else
        funDes(i) = exp(abs(funComp(i)) .* (1 + log(A)) - 1)./A .* sign(funComp(i));
    end
end
